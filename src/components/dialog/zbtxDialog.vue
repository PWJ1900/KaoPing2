    ...
    <el-dialog title="指标体系"
      ...
      <table style="width:900px;text-align:center"
        ...
        <tr>
          ...
          <td colspan="3">
            <el-input v-model="zhibiaotixi.tiximingchengdingyi"></el-input>
          </td>
        </tr>
        <tr>
          ...
          <td colspan="3">
            <el-select v-model="zhibiaotixi.cpxh">
              <el-option v-for="(item , i ) in zhibiaotixi.cepingxuhaoxuanze"
                         :key="i"
                         :label="item"
                         :value="i"></el-option>
            </el-select>
        ...
        <!-- 指标内容 -->
        <tr>
          <td :rowspan="zhibiaoneirong.list.length+1"
              colspan="1">指标内容</td>
          <td colspan="1">二级指标选择：</td>
          <td colspan="1">
            <el-select v-model="zhibiaoneirong.ejzb">
              <el-option v-for="(item , i ) in zhibiaoneirong.erjizhibiaoxuanze"
                         :key="i"
                         :label="item"
                         :value="i"></el-option>
            </el-select>
        ...
        <tr v-for="(item,i) in zhibiaoneirong.list"
          ...
          <td>指标{{i+1}}:</td>
          <td>
            <el-input v-model="zhibiaoneirong.list[i].b" />
          </td>
          <td>
            <el-input v-model="zhibiaoneirong.list[i].c" />
          </td>
          <td>
            <el-input v-model="zhibiaoneirong.list[i].d" />
          </td>
        </tr>
        <!-- 参评群体内容 -->
        <tr>
          <td :rowspan="canpingquntineirong.list.length+1"
              colspan="1">参评群体内容</td>
          <td colspan="1">参评群体选择：</td>
          <td colspan="2">
            <el-select v-model="canpingquntineirong.cpqt">
              <el-option v-for="(item , i ) in canpingquntineirong.canpingquntixuanze"
                         :key="i"
                         :label="item"
                         :value="i"></el-option>
            </el-select>
        ...
        <tr v-for="(item,i) in canpingquntineirong.list"
            :key="i">
          <td colspan="1">群体{{i+1}}:</td>
          <td colspan="2">
            <el-input v-model="canpingquntineirong.list[i].b"></el-input>
          </td>
          <td colspan="1">
            <el-input v-model="canpingquntineirong.list[i].c"></el-input>
          </td>
        </tr>
        <!-- 否决性指标 -->
        <tr>
          <td :rowspan="foujuexingzhibiao.length+1"
              colspan="1">否决性指标</td>
          <td :rowspan="foujuexingzhibiao.length+1"
              colspan="1">指标内容:<br>
            <el-button type="small"
                       @click="fjxzb_add">增加指标内容</el-button><br>
            <el-button type="small"
                       @click="fjxzb_dec">减少指标内容</el-button>
          </td>
          <td colspan="3">具体内容</td>
        </tr>
        <tr v-for="(item,i) in foujuexingzhibiao"
            :key="i">
          <td colspan="3">{{i+1}}、<el-input v-model="foujuexingzhibiao[i]"></el-input>
          </td>
        </tr>
        <!-- 总体评价 -->
        <tr>
          <td :rowspan="zongtipingjia.xuanxiangdingyi.length+3"
              colspan="1">总体评价</td>
        ...
        <tr>
          ...
          <td colspan="3">
            <el-input placeholder="请按照下放要求输入"
                      v-model="zongtipingjia.pingjiabiaoti"></el-input>
            <br>
            <p><b>“文本框型题|选项型标题|上行右边的标题|上行左边的标题” 没有为空，但"|"一定要输入</b></p>
          </td>
        </tr>
        <tr>
          <td :rowspan="zongtipingjia.xuanxiangdingyi.length+1"
              colspan="1">选项定义：
            <el-button type="small"
                       @click="ztpj_add">增加指标内容</el-button><br>
            <el-button type="small"
                       @click="ztpj_dec">减少指标内容</el-button>
          </td>
          <td colspan="3">选项内容</td>
        </tr>
        <tr v-for="(item,i) in zongtipingjia.xuanxiangdingyi"
            :key="i">
          <td colspan="3">
